...
• Kinect-class, Azure Kinect, and RealSense evaluations give centimetre-to-millimetre orders of magnitude that can be used in a design review only after lighting and material are matched to the test.

• Structured light, compact ToF/active stereo, and solid-state LiDAR occupy different range and ambient-light envelopes, so later product generations did not retire the earlier ones.

• Tabletop TSDF fusion, indoor visual-inertial-depth odometry, and outdoor lidar-inertial odometry assume different front-ends; swapping the camera without changing the estimator is a common failure.

• ISO/TS 15066 speed-and-separation logic still needs a safety controller and a redundant channel; a USB RGB-D camera by itself does not provide that integrity.
...
A mobile manipulator, an indoor delivery base, and a small inspection UAV all need a geometric model of the workspace that arrives in time to change a motor command [1,2,3,4]. Cameras are used for that job because they are non-contact and, at factory prices, information-dense. A two-dimensional image gives appearance. An active depth camera or a solid-state ranger adds a metric range, so obstacle clearance, grasp approach, and map scale no longer have to be inferred from texture alone. On a machine that can jam a peg or strike a girder, a millimetre that would be invisible in a visualisation is a control tolerance.

The scope is active depth: the sensor emits a controlled optical source and recovers geometry from the return. We group that hardware into structured light and active stereo, continuous-wave and pulsed ToF cameras, and solid-state LiDAR [5,6,7,8,9,10]. Passive stereo and learned monocular depth are discussed only as contrasts, because they depend on ambient texture or on priors learned from data [11,12,13]. Those passive pipelines have closed much of the accuracy gap in well-textured outdoor video, yet weakly textured indoor parts and lighting that changes during a shift still push many deployed cells toward an illuminator [4,14,15].

Microsoft's Kinect, released in 2010, made dense indoor depth cheap enough that robotics groups could buy several units from a consumer store [14,16]. KinectFusion, RGB-D mapping, and body-pose pipelines followed within a few years and moved those tasks out of a handful of specialised laboratories [17,18,19]. Later modules cut mass and watts (phone-class ToF and active stereo) and, separately, pushed the working distance into tens of metres (solid-state LiDAR), so the same class of measurement began to appear on platforms that could not have carried a 2010 sensor head [9,10,20,21].

Table 1 lists surveys that already treat slices of this hardware in more depth than a single paper can. Foix et al. and Horaud et al. remain the starting points for lock-in and ToF cameras [7,8,22,23]; Zhang and Geng for structured-light metrology [5,6]; Halmetschlager-Funek et al. and Giancola et al. for multi-camera indoor tests [4,24]; Cadena et al. and later visual-SLAM reviews for estimators [1,25,26,27]; Gallego et al. for event cameras [28]; Ho, Roriz, Li, and Raj for LiDAR scanners [9,10,29,30]. The present manuscript uses those sources rather than repeating their derivations. The remaining design question is narrower: which physical generation made which machine function practical, and which residual errors still dominate once the sensor is bolted to a robot.
...
The rule set is enough for a reader to reconstruct the search. It is too weak for a pooled RMSE across consumer cameras: Kinect, Azure Kinect, and RealSense studies use different targets, lighting, and warm-up, so the numbers cannot be averaged without inventing a common experiment [4,24,35,36]. Records in languages other than English, and manufacturer white papers offered as the only evidence, were left out of the primary set. The last database refresh was March 2025, so SKUs announced after that date sit outside the window.

### 1.2 Contributions and organization

The intended reader is a mechatronic designer who already has access to the optics surveys in Table 1. The manuscript (i) compares structured light, active stereo, iToF, dToF, and solid-state LiDAR by principle, reported envelope, and typical role; (ii) gathers device-level numbers from peer-reviewed Kinect-class, RealSense-class, and Azure Kinect tests so that Table 3 can sit in a design review [14,15,35,36,37]; (iii) connects those artifacts to the estimators that actually run on machines, including lidar-inertial and lidar-visual-inertial odometry [38,39,40,41]; (iv) groups applications by the depth property they spend; (v) records, in Table 8, what public benchmarks score and what they leave untested; and (vi) supplies a selection flowchart plus a ten-point checklist that can be copied into a requirements sheet. Hardware is Section 2, estimators Section 3, applications Section 4, open problems Section 5, and a short close Section 6.
...
Consumer and automotive depth images share a small set of artifacts: impulse noise, invalid pixels (holes) on occlusions, black or specular surfaces, flying pixels on depth edges, and ToF multipath [4,7,15]. Firmware still starts with a spatial median and a short temporal average. Where a registered RGB image exists, joint bilateral or guided filtering uses colour edges to protect object boundaries while smoothing planar interiors [69]. Those filters run on modest GPUs; they fill nothing that the camera never measured.

Depth completion does invent geometry. Early methods inpainted by diffusion or multi-view consistency. Networks trained on RGB-D or on RGB plus sparse LiDAR now dominate the published record [12,13,70,71,72,73,74,75,76,77,78]. Eigen et al. showed that a multi-scale network can predict depth from a single RGB image [12]; residual and self-supervised models later reduced the need for dense ground truth [13,71,72]. Indoor work still leans on NYU-Depth v2 apartments [70] and, when more rooms are required, on ScanNet and Matterport3D reconstructions [79,80]. Zhang and Funkhouser completed a single RGB-D frame with holes by predicting surface normals and occlusion boundaries as intermediate cues [75]. Outdoor work leans on KITTI-style projected LiDAR, which is sparse and biased toward the road plane [73,81,82]. Uhrig et al. introduced sparsity-invariant convolutions for that setting [73], and Ma and Karaman showed that a few hundred metric samples plus RGB already yield a dense metric map [74]. Spatial-propagation networks (CSPN, NLSPN) and DeepLiDAR-style normal guidance then became the usual way to spread those sparse seeds [76,77,78]. Later non-local and transformer models fill larger holes by using long-range context, at a cost that still sits awkwardly on a small onboard computer [83]. For a machine that may stop a motor, those networks are useful as a virtual sensor for visualisation and mid-level planning. They remain a poor sole input to a safety-rated stop, because a method that ranks well on KITTI projected LiDAR can still invent a thin pole or a glass door in a warehouse aisle [73,82].
...
Workspace monitoring. A ceiling or cell-side depth sensor builds a three-dimensional occupancy of the shared volume. Speed-and-separation monitoring, as framed by ISO/TS 15066, can then slow or stop the arm when a person enters a warning or protective zone [3,33]. The industrial-robot safety standard ISO 10218 still sits underneath that technical specification [34]. Three-dimensional monitoring is more complete than a light curtain because it can shrink the protective volume as the arm slows and can see a person who leans over a fence. Control papers that quantify collaborative safety treat separation, speed, and a defined protective stop as metrics, not as a camera specification [156]. A USB RGB-D module has no safety rating of its own. Certified cells still add a safety controller, documented failure modes, and usually a second sensing channel [3,33,153,155]. Industrial HRI surveys treat safety and the user interface as the two issues that decide whether a cobot is actually used; a depth demonstration by itself does not settle that [3,152,154]. Collision detection on the arm remains necessary, because a camera cannot see inside the contact [153].

Programming by demonstration. Tracking a tool or a marked workpiece in 3D lets a non-expert move the robot through a path [3,152]. Structured light at short range is usually sufficient. The hard parts are correspondence, occlusion by the operator, and converting a human demonstration into a collision-free, force-feasible program. Gesture and activity cues can distinguish a small set of commands or detect that a station is ready for the next part [3,16]. Those cues still sit behind a hardware enable switch; they do not replace it.
...
Event-based and neuromorphic sensing. Asynchronous pixels report contrast changes at microsecond latency and high dynamic range [28,157,160]. Combined with pulsed illumination they can support low-power depth for high-speed robots [142,143,144,161,162]. Automotive work has already shown that event cameras can close a low-latency perception loop that frame cameras miss [163]. Co-design with spiking processors aims to cut the energy of spatial reasoning [158,159]. In the next product cycle those pipelines are more likely to sit beside frame-based RGB-D than to retire it.
