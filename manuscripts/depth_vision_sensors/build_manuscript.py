...
def manuscript_blocks() -> list[tuple]:
    ...
    b.append((
        "p",
        "Background: Compact structured-light cameras, time-of-flight (ToF) modules, and solid-state LiDAR now appear on the same shop-floor robots, indoor bases, and small unmanned aerial vehicles (UAVs), so a single catalogue line labelled “depth sensor” is no longer a usable specification. Methods: We surveyed English-language hardware, metrology, and application papers from 2001 through March 2025 in IEEE Xplore, Scopus, and Web of Science, keeping records that name the ranging principle and report a measurable error, range, or deployed result. Results: Close-range accuracy still belongs to structured light; room-scale heads are mostly iToF or active stereo; outdoor tens of metres belong to solid-state LiDAR. Those envelopes decide whether a truncated signed distance map, visual-inertial-depth odometry, lidar-inertial odometry, or a certified stop is even a sensible next block. Conclusions: Range, lighting, surface, mass, and safety integrity should be written down before a SKU is chosen, and a lightly filtered metric range path should be kept whenever a motor can stop. The review is a system map; Foix, Zhang, and related optics surveys remain the place to go for device physics."
    ))
    b.append((
        "p",
    ...
    b.append(("p", "• Kinect-class, Azure Kinect, and RealSense evaluations give centimetre-to-millimetre orders of magnitude that can be used in a design review only after lighting and material are matched to the test."))
    b.append(("p", "• Structured light, compact ToF/active stereo, and solid-state LiDAR occupy different range and ambient-light envelopes, so later product generations did not retire the earlier ones."))
    b.append(("p", "• Tabletop TSDF fusion, indoor visual-inertial-depth odometry, and outdoor lidar-inertial odometry assume different front-ends; swapping the camera without changing the estimator is a common failure."))
    b.append(("p", "• ISO/TS 15066 speed-and-separation logic still needs a safety controller and a redundant channel; a USB RGB-D camera by itself does not provide that integrity."))
    ...
    b.append((
        "p",
        f"A mobile manipulator, an indoor delivery base, and a small inspection UAV all need a geometric model of the workspace that arrives in time to change a motor command {cite('cadena2016','floreano2015','villani2018','halmet2019')}. Cameras are used for that job because they are non-contact and, at factory prices, information-dense. A two-dimensional image gives appearance. An active depth camera or a solid-state ranger adds a metric range, so obstacle clearance, grasp approach, and map scale no longer have to be inferred from texture alone. On a machine that can jam a peg or strike a girder, a millimetre that would be invisible in a visualisation is a control tolerance.",
    ))
    b.append((
        "p",
        f"The scope is active depth: the sensor emits a controlled optical source and recovers geometry from the return. We group that hardware into structured light and active stereo, continuous-wave and pulsed ToF cameras, and solid-state LiDAR {cite('zhang2012sl','geng2011','foix2011','horaud2016','ho2022','roriz2022')}. Passive stereo and learned monocular depth are discussed only as contrasts, because they depend on ambient texture or on priors learned from data {cite('scharstein2002','eigen2014','godard2017')}. Those passive pipelines have closed much of the accuracy gap in well-textured outdoor video, yet weakly textured indoor parts and lighting that changes during a shift still push many deployed cells toward an illuminator {cite('khoshelham2012','halmet2019','sarbolandi2015')}.",
    ))
    b.append((
        "p",
        f"Microsoft's Kinect, released in 2010, made dense indoor depth cheap enough that robotics groups could buy several units from a consumer store {cite('khoshelham2012','shotton2011')}. KinectFusion, RGB-D mapping, and body-pose pipelines followed within a few years and moved those tasks out of a handful of specialised laboratories {cite('newcombe2011','izadi2011','henry2012')}. Later modules cut mass and watts (phone-class ToF and active stereo) and, separately, pushed the working distance into tens of metres (solid-state LiDAR), so the same class of measurement began to appear on platforms that could not have carried a 2010 sensor head {cite('keselman2017','gyongy2022','ho2022','roriz2022')}.",
    ))
    b.append((
        "p",
        f"Table 1 lists surveys that already treat slices of this hardware in more depth than a single paper can. Foix et al. and Horaud et al. remain the starting points for lock-in and ToF cameras {cite('foix2011','horaud2016','kolb2010','zanuttigh2016')}; Zhang and Geng for structured-light metrology {cite('zhang2012sl','geng2011')}; Halmetschlager-Funek et al. and Giancola et al. for multi-camera indoor tests {cite('halmet2019','giancola2018')}; Cadena et al. and later visual-SLAM reviews for estimators {cite('cadena2016','taketomi2017','barros2022','wang2024slam')}; Gallego et al. for event cameras {cite('gallego2022')}; Ho, Roriz, Li, and Raj for LiDAR scanners {cite('ho2022','roriz2022','li2020lidar','raj2020')}. The present manuscript uses those sources rather than repeating their derivations. The remaining design question is narrower: which physical generation made which machine function practical, and which residual errors still dominate once the sensor is bolted to a robot.",
    ))
    b.append((
        "table",
        (
            "Table 1. Prior surveys used as starting points, and the integration question left for a mechatronic designer.",
    ...
    b.append((
        "p",
        "We wrote a structured narrative review. No PRISMA protocol was registered, and we do not report a screened-record count, because that numerator was never collected. The search window, the inclusion rule, and the use made of each kept paper are stated so that a reader can see how the corpus was bounded.",
    ))
    b.append((
        "p",
    ...
    b.append((
        "p",
        f"The rule set is enough for a reader to reconstruct the search. It is too weak for a pooled RMSE across consumer cameras: Kinect, Azure Kinect, and RealSense studies use different targets, lighting, and warm-up, so the numbers cannot be averaged without inventing a common experiment {cite('halmet2019','giancola2018','tolgyessy2021','kurillo2022')}. Records in languages other than English, and manufacturer white papers offered as the only evidence, were left out of the primary set. The last database refresh was March 2025, so SKUs announced after that date sit outside the window.",
    ))

    b.append(("h2", "1.2 Contributions and organization"))
    b.append((
        "p",
        f"The intended reader is a mechatronic designer who already has access to the optics surveys in Table 1. The manuscript (i) compares structured light, active stereo, iToF, dToF, and solid-state LiDAR by principle, reported envelope, and typical role; (ii) gathers device-level numbers from peer-reviewed Kinect-class, RealSense-class, and Azure Kinect tests so that Table 3 can sit in a design review {cite('khoshelham2012','sarbolandi2015','carfagni2019','tolgyessy2021','kurillo2022')}; (iii) connects those artifacts to the estimators that actually run on machines, including lidar-inertial and lidar-visual-inertial odometry {cite('zhang2014loam','shan2020','xu2022fastlio','shan2021lvisam')}; (iv) groups applications by the depth property they spend; (v) records, in Table 8, what public benchmarks score and what they leave untested; and (vi) supplies a selection flowchart plus a ten-point checklist that can be copied into a requirements sheet. Hardware is Section 2, estimators Section 3, applications Section 4, open problems Section 5, and a short close Section 6.",
    ))
    ...
    b.append((
        "p",
        "Table 2 gathers order-of-magnitude envelopes from the cited evaluations and hardware surveys. A serial number can sit well outside those bands, so the table is a starting envelope rather than an acceptance sheet. Two consequences follow for a bill of materials. A camera that is excellent above a bin is usually the wrong camera in a 30 m aisle, which is why the phrase “depth camera” is a poor specification by itself. Later product waves also failed to retire earlier ones: structured light still holds close-range accuracy, compact ToF holds room-scale heads, and solid-state LiDAR holds outdoor range. Mixed machines therefore tend to carry more than one family.",
    ))
    b.append((
        "table",
        (
    ...
    b.append((
        "p",
        "Figure 3 is the causal claim of the review drawn as a timeline: each hardware step made a machine function practical at a new size and cost, while the older function remained in service on the platforms that still needed it.",
    ))
    ...
    b.append((
        "p",
        "A raw depth image is still only a range field. Pose, mesh, grasp, and stop commands are produced by the software layers reviewed in this section.",
    ))

    b.append(("h2", "3.1 Depth data processing and enhancement"))
    b.append((
        "p",
        f"Consumer and automotive depth images share a small set of artifacts: impulse noise, invalid pixels (holes) on occlusions, black or specular surfaces, flying pixels on depth edges, and ToF multipath {cite('foix2011','halmet2019','sarbolandi2015')}. Firmware still starts with a spatial median and a short temporal average. Where a registered RGB image exists, joint bilateral or guided filtering uses colour edges to protect object boundaries while smoothing planar interiors {cite('kopf2007')}. Those filters run on modest GPUs; they fill nothing that the camera never measured.",
    ))
    b.append((
        "p",
        f"Depth completion does invent geometry. Early methods inpainted by diffusion or multi-view consistency. Networks trained on RGB-D or on RGB plus sparse LiDAR now dominate the published record {cite('silberman2012','eigen2014','laina2016','godard2017','godard2019','uhrig2017','ma2018','zhang2018ddc','cheng2018cspn','park2020nlspn','qiu2019deeplidar')}. Eigen et al. showed that a multi-scale network can predict depth from a single RGB image {cite('eigen2014')}; residual and self-supervised models later reduced the need for dense ground truth {cite('laina2016','godard2017','godard2019')}. Indoor work still leans on NYU-Depth v2 apartments {cite('silberman2012')} and, when more rooms are required, on ScanNet and Matterport3D reconstructions {cite('dai2017scannet','chang2017matterport')}. Zhang and Funkhouser completed a single RGB-D frame with holes by predicting surface normals and occlusion boundaries as intermediate cues {cite('zhang2018ddc')}. Outdoor work leans on KITTI-style projected LiDAR, which is sparse and biased toward the road plane {cite('geiger2012kitti','geiger2013kitti','uhrig2017')}. Uhrig et al. introduced sparsity-invariant convolutions for that setting {cite('uhrig2017')}, and Ma and Karaman showed that a few hundred metric samples plus RGB already yield a dense metric map {cite('ma2018')}. Spatial-propagation networks (CSPN, NLSPN) and DeepLiDAR-style normal guidance then became the usual way to spread those sparse seeds {cite('cheng2018cspn','park2020nlspn','qiu2019deeplidar')}. Later non-local and transformer models fill larger holes by using long-range context, at a cost that still sits awkwardly on a small onboard computer {cite('guo2021pc')}. For a machine that may stop a motor, those networks are useful as a virtual sensor for visualisation and mid-level planning. They remain a poor sole input to a safety-rated stop, because a method that ranks well on KITTI projected LiDAR can still invent a thin pole or a glass door in a warehouse aisle {cite('uhrig2017','geiger2013kitti')}.",
    ))
    b.append((
        "p",
        "Table 4 rates the same artifacts by how mature the mitigation is on shipped hardware. Learned completion can look complete while remaining metrically wrong on glass, thin stock, or objects absent from the training set, which is why we keep it off the motor-stop path.",
    ))
    b.append((
        "table",
        (
    ...
    b.append((
        "p",
        f"Workspace monitoring. A ceiling or cell-side depth sensor builds a three-dimensional occupancy of the shared volume. Speed-and-separation monitoring, as framed by ISO/TS 15066, can then slow or stop the arm when a person enters a warning or protective zone {cite('iso15066','villani2018')}. The industrial-robot safety standard ISO 10218 still sits underneath that technical specification {cite('iso10218')}. Three-dimensional monitoring is more complete than a light curtain because it can shrink the protective volume as the arm slows and can see a person who leans over a fence. Control papers that quantify collaborative safety treat separation, speed, and a defined protective stop as metrics, not as a camera specification {cite('zanchettin2016')}. A USB RGB-D module has no safety rating of its own. Certified cells still add a safety controller, documented failure modes, and usually a second sensing channel {cite('villani2018','haddadin2017','iso15066','robla2017')}. Industrial HRI surveys treat safety and the user interface as the two issues that decide whether a cobot is actually used; a depth demonstration by itself does not settle that {cite('villani2018','ajoudani2018','lasota2017')}. Collision detection on the arm remains necessary, because a camera cannot see inside the contact {cite('haddadin2017')}.",
    ))
    b.append((
        "p",
        f"Programming by demonstration. Tracking a tool or a marked workpiece in 3D lets a non-expert move the robot through a path {cite('villani2018','ajoudani2018')}. Structured light at short range is usually sufficient. The hard parts are correspondence, occlusion by the operator, and converting a human demonstration into a collision-free, force-feasible program. Gesture and activity cues can distinguish a small set of commands or detect that a station is ready for the next part {cite('shotton2011','villani2018')}. Those cues still sit behind a hardware enable switch; they do not replace it.",
    ))
    ...
    b.append((
        "p",
        "Published algorithms sometimes describe leftover ranging error as a temporary software problem. Fielded cells suggest a slower picture. Table 7 splits mitigations that already ship from ideas that remain in the laboratory.",
    ))
    b.append((
        "p",
    ...
    b.append((
        "p",
        "Depth vision reached factory robots, indoor bases, and small UAVs through three overlapping hardware waves rather than through a single replacement chain. Structured light made dense, inexpensive, close-range geometry ordinary. ToF shrank that measurement onto mobile heads and phones. Solid-state LiDAR then made long-range ranging compatible with vibration-sensitive airframes. Completion, tight-coupled odometry, RGB-D SLAM, and 3D semantics converted those measurements into poses, maps, grasps, and safety zones.",
    ))
    b.append((
        "p",
        "What still limits deployment is mostly physics and organisation: sunlight and materials, watts and memory, calibration drift, and coordinate frames that differ by vendor. The next useful stack is therefore unlikely to be one universal camera. It is more likely to be a short set of complementary ranges, an estimator that reports when it should be ignored, and, where the factory allows it, a shared map that each shift does not have to rebuild.",
    ))
    b.append((
        "p",
        "The practical close is conservative. Pick the family from the operating envelope rather than from the marketing generation. Put calibration, lighting, and a warm-up interval on the same budget line as the camera. Keep a lightly filtered metric range path whenever the output can stop a motor. Use public benchmarks to see where a method was tested, then repeat the test on the surfaces, lighting, and safety integrity of the actual cell or airframe.",
    ))
